...
uintptr_t getPlayer(int index) {  //Each player in the game has an index.
	return RPM< uintptr_t>(moduleBase + hazedumper::signatures::dwEntityList + index * 0x10); //We use index times 0x10 because the distance between each player 0x10.
}
...
int getCrosshairID(uintptr_t player) {
	return RPM<int>(player + hazedumper::netvars::m_iCrosshairId);
}
